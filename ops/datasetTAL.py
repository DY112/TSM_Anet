...
class TALDataSet(data.Dataset):
    ...
    def _parse_bbdb(self):
        ...
        assert subset in ('train', 'validation', 'test', 'otal_test'), 'Unknown BBDB Subset!! (train | val | test | otal_test)'
        ...
        if subset in ('train', 'validation', 'test'):
            with open(self.meta_dict['annt_file'], 'r') as json_file:
                ...
                for video_name in database:
                    ...
                    # Check if h5 is fps 6
                    H5_FPS = 6
                    h5_file = h5py.File(path, 'r')
                    assert abs(h5_file['jpegs'].shape[0] - video_info['duration'] * H5_FPS) < 5, 'Vid: %s, h5_len: %d, vid_dur: %f'%(video_name, h5_file['jpegs'].shape[0], video_info['duration'] * H5_FPS)
                    
                    for segment in video_info["annotations"]:
                        st = segment["segment"][0]
                        et = segment["segment"][1]
                        label = segment["labelIndex"] + 1 # Background = 0
                        
                        annot = [path, st, et, label]
                        annots.append(annot)

        elif subset is 'otal_test':
            with open(self.meta_dict['annt_file'], 'r') as json_file:
                json_data = json.load(json_file)
        ...
        print ('Subset: %s, Num_vid: %d' % (subset, num_vid))
    ...
    def _temporal_jittering(self, org_s, org_e):
        ...
        while True:
            ...
            # Apply strict constraint on BBDB
            if self.dataset != 'BBDB' or org_e - end_point < len_seg * 0.2:
                break
    ...
    def get_fr(self, record):
        if self.dataset in ('ANet', 'Thumos'):
            ...
            start_frame_6fps = int(record.start_sec * 6)
            end_frame_6fps = int(record.end_sec *  6)
...
if __name__ == '__main__':
    ...
    BBDB_VID_PATH = '/workspace/BBDB_6fps/videos/'

    subset = 'train'  # 'validation'  'test'
    bbdb_dataset = TALDataSet(BBDB_VID_PATH, 
                   {'classidx':BBDB_CLASSIDX, 'annt_file':BBDB_ANNOTATION_FILE, 'subset':subset},
                   'BBDB', 
                   num_segments=8,
                   new_length=1,
                   modality='RGB',
